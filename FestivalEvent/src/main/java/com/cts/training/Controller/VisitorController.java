...
public class VisitorController {
	...
	public ModelAndView registerVisitor() {
		System.out.println("Visitor Details are Registered");
	...
	System.out.println("The Entered details are successfully saved in database");
		theModel.addAttribute("Login", new VisitorLoginBean());
		VisitorEntity user = service.getVisitorObject(visitorBean.getUserName());
		if (user != null) {
	...
	public String setupForm(Model model) {
		VisitorLoginBean Login = new VisitorLoginBean();
	...
	public String submitForm(@ModelAttribute("Login") VisitorLoginBean Login, BindingResult result,
			SessionStatus status, Model theModel) {
		
		boolean mismatch = false;

		System.out.println(Login); 
		if (Login.getUserName().isEmpty() == true || Login.getPassWord().isEmpty() == true) {
			if (Login.getUserName().isEmpty()) {
				result.rejectValue("userName", "mismatch.userName");
				mismatch = true;
				return "login";
			}
			if (Login.getPassWord().isEmpty()) {
				result.rejectValue("passWord", "mismatch.passWord");
				mismatch = true;
				return "login";
			}
		}
		if ((Login.getUserName().isEmpty()) == false && (Login.getPassWord().isEmpty()) == false) {
			String out = service.validateLogin(Login.getUserName(), Login.getPassWord());

			if (out.equals("notFound")) {
				result.rejectValue("userName", "mismatch.User not Registered");
				mismatch = true;
				return "login";
			}
			if (out.equals("incorrect")) {
				result.rejectValue("passWord", "mismatch.Password is not correct");
				mismatch= true;
				return "login";
			}
		}
		...
		theModel.addAttribute("visitor", service.getVisitorObject(Login.getUserName()));
		theModel.addAttribute("allEvents", eService.showEvent());
		List<EventEntity> list = eService.getRegisteredEvent(service.getId(Login.getUserName()));
		if (list.isEmpty()) {
	...
	public String updateValues(@ModelAttribute("registrationAttribute") VisitorRegBean visitorBean, Model theModel) {
		theModel.addAttribute("visitor", service.updateVisitor(visitorBean));
...
//UpdatePassword
	...
	public ModelAndView updatePwd(@RequestParam("status") String userName, @RequestParam("password") String pwd,
		...
		if (out == true) {
			...
			theModel.addAttribute("message", "unable to change password");
	...
	/*
	 * @RequestMapping(value = "/logout.html", method = RequestMethod.GET) public
	 * String logoutForm(Model model) { VisitorLoginBean Login = new
	 * VisitorLoginBean();
	 * 
	 * model.addAttribute("Logout", Logout); return "index"; }
	 */
